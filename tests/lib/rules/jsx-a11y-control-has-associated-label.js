...
ruleTester.run(`${ruleName}:recommended`, rule, {
  valid: [...alwaysValid].map(ruleOptionsMapperFactory(recommendedOptions)),
  invalid: [...neverValid].map(ruleOptionsMapperFactory(recommendedOptions)),
});

const strictOptions = configs.strict.rules[ruleName][1] || {};

ruleTester.run(`${ruleName}:strict`, rule, {
  valid: [...alwaysValid].map(ruleOptionsMapperFactory(strictOptions)),
  invalid: [...neverValid].map(ruleOptionsMapperFactory(strictOptions)),
});
...
ruleTester.run(`${ruleName}:no-config`, rule, {
  ...
  invalid: [{ code: '<input type="text" />', errors: expectedErrors }],
});
