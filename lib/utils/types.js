...
function createHasListField(fieldName) {
    return (node = {}) => Array.isArray(node[fieldName]) && node[fieldName].length > 0;
}
...
function isExportStatement(node) {
    return t.isExportDefaultDeclaration(node) || t.isExportNamedDeclaration(node) || t.isExportAllDeclaration(node);
}
...
function isReactFunctionComponent(node) {
    return node.left.name === "React" &&
          (node.right.name === "FC" || node.right.name === "FunctionComponent" || node.right.name === "VFC" || node.right.name === "VoidFunctionComponent" || node.right.name === "StatelessComponent");
}
