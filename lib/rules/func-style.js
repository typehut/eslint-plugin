...
const errors = {
    ...
    declarationInTopLevel: "Expected a function declaration in top-level context."
};
...
function create(context) {
    const PREFIX_COLON = /^:\s*/u,
        ...
        AVAILABLE_CONST = normalizeEcmaVersion(context.parserOptions.ecmaVersion || 5) >= 2015,
    ...
    function analyze(node, parent) {
        ...
        const result = {
            ...
            params: ((isFunctionDeclaration ? node.params : node.init.params) || []).map(param => sourceCode.getText(param)).join(", ") || null,
            typeAnnotation: null,
            typeParameters: null,
            returnType: null
        };
        ...
        if (isFunctionDeclaration) {
            ...
            result.typeParameters = node.typeParameters && node.typeParameters.params && node.typeParameters.params.length ? node.typeParameters.params.map(param => sourceCode.getText(param)).join(", ") : null;
        } else {
            ...
            result.typeParameters = node.init.typeParameters && node.init.typeParameters.params && node.init.typeParameters.params.length ? node.init.typeParameters.params.map(param => sourceCode.getText(param)).join(", ") : null;
    ...
    function generateFixedText(data, config) {
        ...
        switch (to) {
            case "declaration":
                return `${data.async ? "async " : ""}function${data.generator ? "* " : ""}${data.name ? ` ${data.name}` : ""}${data.typeParameters ? `<${data.typeParameters}>` : ""}(${data.params ? data.params : ""})${data.returnType ? `: ${data.returnType}` : ""} ${t.isBlockStatement(data.body) ? "" : `{return ${data.bodyText}}`}`;
            case "expression":
                return `${data.kind} ${data.name}${data.typeAnnotation ? `: ${data.typeAnnotation}` : ""} = ${data.async ? "async " : ""}function${data.generator ? "* " : ""}${data.typeParameters ? `<${data.typeParameters}${IS_GENERIC_PARSEABLE.test(data.typeParameters) ? "" : ","}>` : ""}(${data.params ? data.params : ""})${data.returnType ? `: ${data.returnType}` : ""} ${t.isBlockStatement(data.body) ? "" : `{return ${data.bodyText}}`}`;
            case "arrow":
                return `${data.kind} ${data.name}${data.typeAnnotation ? `: ${data.typeAnnotation}` : ""} = ${data.async ? "async " : ""}${data.generator ? "* " : ""}${data.typeParameters ? `<${data.typeParameters}${IS_GENERIC_PARSEABLE.test(data.typeParameters) ? "" : ","}>` : ""}(${data.params ? data.params : ""})${data.returnType ? `: ${data.returnType}` : ""} => ${t.isBlockStatement(data.body) ? "" : data.bodyText}`;
            default:
                return null;
        }
    }
    ...
    function report({ node, messageId, isTopLevel }) {
        context.report({
            ...
            fix: disableAutoFix ? null : fixer => {
                ...
                const range = t.isBlockStatement(data.body) ? [data.defStart, data.defEnd] : [data.start, data.end];
    ...
    return {
        FunctionDeclaration(node) {
            ...
            if (!isTopLevel && !enforceDeclarations && (!t.isExportDefaultDeclaration(node.parent) || !allowExportDefaultFunctionDeclaration)) {
                report({ node, messageId: style, isTopLevel });
            }

            if (isTopLevel && !enforceDeclarationTopLevel && (!t.isExportDefaultDeclaration(node.parent) || !allowExportDefaultFunctionDeclaration)) {
        ...
        FunctionExpression(node) {
            ...
            if (!isTopLevel && !enforceExpressions && t.isVariableDeclarator(node.parent) && (!enforceArrows || !isGenerator)) {
                report({ node: node.parent, messageId: style, isTopLevel });
            }

            if (isTopLevel && !enforceExpressionTopLevel && t.isVariableDeclarator(node.parent) && (!enforceArrowTopLevel || !isGenerator)) {
                report({ node: node.parent, messageId: `${topLevelStyle}InTopLevel`, isTopLevel });
            }
        },
        ...
        ArrowFunctionExpression(node) {
            ...
            if (!isTopLevel && !enforceArrows && t.isVariableDeclarator(node.parent)) {
                report({ node: node.parent, messageId: style, isTopLevel });
            }

            if (isTopLevel && !enforceArrowTopLevel && t.isVariableDeclarator(node.parent)) {
                report({ node: node.parent, messageId: `${topLevelStyle}InTopLevel`, isTopLevel });
            }
        },
