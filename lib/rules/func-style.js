...
module.exports = {
    ...
    create(context) {
        const PREFIX_COLON = /^:\s*/u,
            ...
            disabledAutoFix = context.options[1] && context.options[1].disabledAutoFix,
            stack = [];
        ...
        function generateFixedText(data, to) {
            switch (to) {
                case "declaration":
                    return `function${data.name ? ` ${data.name}` : ""}${data.typeParameters ? `<${data.typeParameters}>` : ""}(${data.params ? data.params : ""})${data.returnType ? `: ${data.returnType}` : ""}`;
                case "expression":
                    return `${data.kind} ${data.name} = function${data.typeParameters ? `<${data.typeParameters}>` : ""}(${data.params ? data.params : ""})${data.returnType ? `: ${data.returnType}` : ""}`;
                case "arrow":
                    return `${data.kind} ${data.name} = ${data.typeParameters ? `<${data.typeParameters}>` : ""}(${data.params ? data.params : ""})${data.returnType ? `: ${data.returnType}` : ""} =>`;
                default:
                    return null;
            }
        }
         ...
         * @param {{node: ASTNode, messageId: string, isTopLevel: boolean, kind: string}} node options
         * @returns {void}
         */
        function report({ node, messageId, isTopLevel, kind }) {
            context.report({
                node,
                messageId,

                fix: disabledAutoFix ? null : fix(fixer) {
        ...
        const nodesToCheck = {
            FunctionDeclaration(node) {
                ...
                if (!isTopLevel && !enforceDeclarations && (node.parent.type !== "ExportDefaultDeclaration" || !allowExportDefaultFunctionDeclaration)) {
                    report({ node, messageId: style, isTopLevel, kind: "FunctionDeclaration" });
                }

                if (isTopLevel && !enforceDeclarationTopLevel && (node.parent.type !== "ExportDefaultDeclaration" || !allowExportDefaultFunctionDeclaration)) {

                    report({ node, messageId: `${topLevelStyle}-in-top-level`, isTopLevel, kind: "FunctionDeclaration" });
                }
            },
            ...
            FunctionExpression(node) {
                ...
                if (!isTopLevel && !enforceExpressions && node.parent.type === "VariableDeclarator") {
                    report({ node: node.parent, messageId: style, isTopLevel, kind: "FunctionExpression" });
                }

                if (isTopLevel && !enforceExpressionTopLevel && node.parent.type === "VariableDeclarator") {

                    report({ node: node.parent, messageId: `${topLevelStyle}-in-top-level`, isTopLevel, kind: "FunctionExpression" });
                }
            },
            ...
            ArrowFunctionExpression(node) {
                ...
                if (!isTopLevel && !enforceArrows && node.parent.type === "VariableDeclarator") {
                    report({ node: node.parent, messageId: style, isTopLevel, kind: "ArrowFunctionExpression" });
                }

                if (isTopLevel && !enforceArrowTopLevel && node.parent.type === "VariableDeclarator") {
                    report({ node: node.parent, messageId: `${topLevelStyle}-in-top-level`, isTopLevel, kind: "ArrowFunctionExpression" });
                }
            },
