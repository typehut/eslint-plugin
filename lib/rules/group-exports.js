 ...
 */
function createNodeListToRemoveProcessor({ context, nodes, elementsToBeRemoved, remainDeclarations }) {
...
function normalizeExports(specifierSet) {
    ...
    specifierSet.forEach(specifier => {
        exports.add(t.isAliasedExportSpecifier(specifier) ? `${specifier.local.name} as ${specifier.exported.name}` : specifier.local.name);
    });
