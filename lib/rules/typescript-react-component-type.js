...
function isFunctionComponent(node) {
    return node.right.name === "FunctionComponent" || node.right.name === "StatelessComponent";
}
...
function createFix(context, node, type) {
    ...
    const rightOperandToken = sourceCode.getFirstToken(node, { filter: token => token.type === "Identifier" && token.value === node.right.name });
